...
void FindE(char A[][4], char target[]){
    ...
    int func[3]={1,27,233};
    printf("\n===================FIND===================\n");
    for(int i=0;target[i]!='\0';i++) key+=(target[i]-'a'+1)*func[i]+i; //hash compution;
    key=key%100;
    if(A[key][0] != '\0'){
        printf("find at: %d.\n", key);
...
void HashA(char A[][4], char ary[][4], int N){
    ...
    int record[100]={0}, func[3]={1,27,233};
    printf("\n===================KEY===================\n");
    for(int i=0;i<N;i++){
        if(i%5==0) printf("%d:\t",i+1);
        for(int j=0;ary[i][j]!='\0';j++) index+=(ary[i][j]-'a'+1)*func[j]+j; //hash compution;
        index=index%100;
        printf("%d\t",index);
        if((i+1)%5==0) printf("\n");
        record[index]++;
