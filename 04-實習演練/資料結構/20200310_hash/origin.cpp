...
int main(void){
    ...
    while(times){
        ...
        for(int i=0;i<times;i++){
            for(int i=0;i<27000;i++) memset(A[i],'\0',4); //init A[][];
            ...
            FindE(A, test);  //在A中 找 目標字串[];
            printf("\n===================END===================\n");
        }
        if(times) printf("\n________ 平均碰撞次數: %lf ________\n",colision/times-15); //平均產生約15個重複字串;
    }
    printf("\n**************程式執行結束**************\n");
    return 0;
}
...
void HashA(char A[][4], char ary[][4], int N){
    ...
    int record[27000]={0}, func[3]={1,27,729};
    printf("\n===================KEY===================\n");
    for(int i=0;i<N;i++){
